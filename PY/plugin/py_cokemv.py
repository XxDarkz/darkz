...
class Spider(Spider):  # 元类 默认的元类 type
	...
	def categoryContent(self,tid,pg,filter,extend):
		...
		rsp = self.fetch(url)
		root = self.html(rsp.text)
		aList = root.xpath("//a[contains(@class,'module-poster-item module-item')]")
		videos = []
		for a in aList:
			name = a.xpath('./@title')[0]
			pic = a.xpath('.//img/@data-original')[0]
	...
	def detailContent(self,array):
		...
		type = ', '.join(typ)
		infoArray = divContent.xpath(".//div[@class='module-info-items']/div[@class='module-info-item']")
		for info in infoArray:
			content = info.xpath('string(.)')
			if content.startswith('导演'):
				dir = content.replace("导演：", "").replace("\r", "").replace("\n", "")
			if content.startswith('主演'):
				actor = content.replace("主演：", "").replace("\r", "").replace("\n", "")
		pic = root.xpath(".//div[@class='module-item-pic']//img/@data-original")[0]
		detail = root.xpath(".//div[@class='module-info-introduction-content show-desc']/p/text()")[0]
		vod = {
			"vod_id":tid,
			"vod_name":title,
			"vod_pic":pic,
	...
	config = {
		"player": {"cokebb":{"show":"极速路线","des":"","ps":"0","parse":""},"mp4":{"show":"蓝光路线","des":"","ps":"0","parse":""},"cokemv001":{"show":"海外线路","des":"","ps":"0","parse":""},"ddzy":{"show":"蓝光采集","des":"","ps":"0","parse":""},"tkm3u8":{"show":"采集线路","des":"","ps":"0","parse":""},"xin":{"show":"高速路線","des":"","ps":"0","parse":""},"90mm":{"show":"COKEMV(測試)","des":"","ps":"0","parse":""},"if101":{"show":"海外路線","des":"","ps":"0","parse":""},"age01":{"show":"動漫一線","des":"","ps":"0","parse":""},"mahua":{"show":"海外(禁國內)","des":"","ps":"0","parse":""},"age02":{"show":"動漫二線","des":"","ps":"0","parse":""}},
		"filter": {}
